...
ostree init --repo=${FLATPAK_SYSTEM_DIR}/repo --mode=archive-z2

for i in $(ostree refs --repo=${FLATPAK_SYSTEM_DIR}/repo | grep '^deploy/' | sed 's/^deploy\///g'); do
  echo "Copying $i..."
  ostree --repo=${FLATPAK_SYSTEM_DIR}/repo pull-local ${FLATPAK_SYSTEM_DIR}/repo $(ostree --repo=${FLATPAK_SYSTEM_DIR}/repo rev-parse ${FLATPAK_REMOTE_NAME}/$i)
...
ostree refs --repo=${FLATPAK_SYSTEM_DIR}/repo | tee /flatpak_dir/list.txt
